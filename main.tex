...
Note that if $\{G_{i}\}_{i\in I}$ is a set of generators and arbitrary coproducts exist in $\C$, then $G=\coprod_{i\in I}G_{i}$ is a generator.
...
Then $P$ is a compact\footnote{In the context of additive categories $\C$, an object $C$ is called \textit{compact} if $\Hom_{\C}(C,-)\colon \C\to \Ab$ preserves coproducts, i.e. for every set $\{A_{i}\}_{i\in I}\subseteq \Ob(\C)$ such that $\coprod_{i}A_{i}$ exists in $\C$, the canonical map $\coprod_{i}\Hom_{\C}(C,A_{i})\to \Hom_{\C}(C,\coprod_{i}A_{i})$ is a group isomorphism.} object in the derived category, because $\Hom_{\D}(P,A\grd)=H^{0}(\Hom_{\Cond(\Ab)}(P,A\grd))$ and cohomology commutes with direct sums in $\Ab$.
