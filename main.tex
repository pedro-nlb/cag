...
\begin{thm}\label{thm:abelian}
    ...
    As Mac Lane points out, the word \textit{separated} would have been a better choice (see \cite[Section V.7]{mac78}).} by compact\footnote{An object $M$ on a category $\C$ is called \textit{compact} if $\Hom_{\C}(M,-)$ commutes with filtered colimits.} projective objects.
    \begin{proof}
...
To describe this triangulated structure we need the following:

\begin{defn}
    Let $A\grd$ be a cochain complex.
    Define its \textit{shift} as the complex $A[p]\grd$ with $A[p]^{i}=A^{i+p}$ and with differential $d_{A[p]}=(-1)^{p}d_{A}$.

    Define the \textit{mapping cone} of a morphism of cochain complexes $f\colon A\grd\to B\grd$ as the complex $C(f)\grd$ with $C(f)^{i}=A^{i+1}\op B^{i}$ and with differential $d_{C(f)}=\bigl( \begin{smallmatrix} d_{A[1]} & 0 \\ f & d_{B} \end{smallmatrix} \bigr)$.
\end{defn}

We take the shift $[1]$ as our suspension functor and we take distinguished triangles to be all triangles isomorphic to one of the form
\[A\grd\xrightarrow{f} B\grd\to C(f)\grd \to A[1]\grd. \]

Because of the cone short exact sequence (see e.g. \cite{wei94}) every distinguished triangle comes from a short exact sequence of complexes.
And conversely, every short exact sequence of complexes gives rise to a distinguished triangle in $\D$ (but not on the homotopy category).
...
\begin{rem}
    ...
    But in practice they are also very inconvenient because of their lack of stability with respect to many usual categorical constructions, which in turn usually goes back to the non-functoriality of cones.
...
We regard $P\in \D$ by seeing it as a complex concentrated on degree $0$.
Then $P$ is a compact\footnote{In the context of additive categories $\C$, an object $C$ is called compact if $\Hom_{\C}(C,-)\colon \C\to \Ab$ preserves coproducts, i.e. for every set $\{A_{i}\}_{i\in I}\subseteq \Ob(\C)$ such that $\op_{i}A_{i}$ exists in $\C$, the canonical map $\op_{i}\Hom_{\C}(C,A_{i})\to \Hom_{\C}(C,\op_{i}A_{i})$ is a group isomorphism.} object in the derived category.
Since being compact is shift invariant and $\Cond(\Ab)$ is compactly generated, so is $\D$.
...
Note that if we already know $F$ to be triangulated, then this theorem is much stronger than the usual adjoint functor theorems, because we only need to check that the functor preserves coproducts.
